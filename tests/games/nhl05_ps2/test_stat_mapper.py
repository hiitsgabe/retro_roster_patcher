...
def test_the_first_defence_pair_is_the_five_on_three_first_unit():
    # `31LD`/`31RD` in the TDB's own spelling, which is the naming the source
    # transcribed. Whether it is the pair NHL 2005 puts on the ice at even
    # strength is not something this repository can establish.
...
def test_the_third_defence_pair_is_emitted_as_thirty_three():
    # THE INHERITED DEFECT, at the layer that produces it. The mapper emits
    # `33LD`/`33RD` and `rom_writer.LINE_FLAGS` names neither, so the writer
    # drops them and this game never assigns a third pair.
    # `tests/games/nhl05_ps2/test_rom_writer.py` pins the other half.

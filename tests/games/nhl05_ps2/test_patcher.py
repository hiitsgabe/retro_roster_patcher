...
Thirty patchable slots, not NHL 07's thirty-two: `SEA` and `VGK` map to the two
All-Star sides and are then dropped, so this game never writes them. And one ROST
mirror, not two: there is no `nhlbioatt.tdb`, so every bio and attribute write
happens once.
...
class FakeApi:
    """Stands in for `EspnClient` and `NhlApiClient`, recording every call.

    `season` reaches the two ESPN endpoints differently -- a cache key on the
    squad call, a URL path segment on the leaders call.
    """
...
def test_thirty_slots_are_patchable():
    # NHL 07 patches 32. The source wrote `slot >= 30` here and `slot >= 32` there,
    # and it agrees with the reader's `INDX > 29` filter.
...
def test_analyzing_a_file_that_is_not_this_game_does_not_raise(tmp_path):
    # `analyze` probes every registered patcher against one image: `RomError` for
    # unreadable, `is_valid=False` for readable-but-not-mine.
...
def test_an_over_declared_extent_fails_the_bound(tmp_path):
    # The directory record claims a longer archive than the image holds, which is
    # what a bad disc read produces. The over-declaration has to reach past the
    # padding file as well, or the extent still lands inside the image.
...
def test_an_over_declaration_inside_the_image_still_satisfies_the_bound(tmp_path):
    # The bound checks one thing -- that the declared extent lies inside the file --
    # so a declaration that runs into the gap and the padding file is accepted.
    natural = len(fixture.build_db_viv())
...
def test_patch_refuses_a_truncated_archive(tmp_path):
    # The asymmetry, pinned: an arithmetic bound guards both entry points, because a
    # file failing it provably cannot be patched. Upstream at this exact length
    # reported success with 25 players written.
    path = iso(tmp_path)
    truncate_to(path, extent_end(path) - 1)
...
def test_the_heuristic_guards_analyze_only(tmp_path):
    # A disc whose `nhl2005.tdb` is not a TDB. `patch` reaches the same fact through
    # `_parse_tdbs`, whose message names the archive's actual file list, so the
    # heuristic deliberately does not reach `patch`.
...
def test_patch_on_the_same_file_fails_with_a_message_about_the_archive(tmp_path):
    # The pair, reached by different routes. `EaTdbError` is a `RomError`.
    spec = fixture.DiscSpec(master_payload=b"NOT A TDB AT ALL" * 512)
    path = iso(tmp_path, spec)
    patcher = build(tmp_path, FakeApi())
    rosters = patcher.map_rosters(league())
...
def test_the_live_range_is_bounded_by_the_allocation(tmp_path):
    ...
    # deliberately does not clamp; this is the consumer's policy for it.
    table = a_table(tmp_path, "ROST")
...
def test_the_live_range_follows_an_understated_count(tmp_path):
    # The other argument of the `min`, so a bound that always answered `capacity`
    # fails here.
    table = a_table(tmp_path, "ROST")
    table.num_records = 7
...
def test_an_index_map_drops_a_zero_index(tmp_path):
    # Zero is what an unused row holds; mapping it would make every unused row look
...
def test_the_play_map_keeps_a_zero_index(tmp_path):
    # Unlike `_index_map`, and the source did the same: a PLAY row is looked up by a
    # ROST row's `INDX`, and if both are zero the source paired them.
    table = a_table(tmp_path, "PLAY")
...
def test_the_play_map_is_not_the_identity(tmp_path):
    # If it were, every test that walks the chain would pass against a patcher that
    # used a record's position for its index.
    table = a_table(tmp_path, "PLAY")
    mapping = _play_id_by_indx(table)
...
def test_the_espn_squad_call_carries_the_season(tmp_path):
    # The endpoint has no season in its URL but does have one in its cache key, so
...
def test_the_espn_leaders_call_carries_the_season(tmp_path):
    # The season is a URL path segment here, so its default meant a `--season 2024`
    # run asked ESPN for another year's statistics and stapled them to the squad.
    api = FakeApi()
    build(tmp_path, api).fetch(season=2021)
...
def test_leaders_travel_in_the_roster_rather_than_on_the_patcher(tmp_path):
    # On `TeamRoster.extra`, not `self.team_stats`: the instance side channel let
    # `fetch` and `map_rosters` run out of order and silently downgrade every player
    # to defaults. Keyed by team *id*, because the ESPN branch calls both endpoints
    # with it.
...
def test_seattle_maps_to_a_slot_and_is_then_dropped(tmp_path):
    # `MODERN_NHL_TO_NHL05["SEA"]` is 30, an All-Star side, and this game does not
    # patch those, so the team is fetched, mapped and discarded. NHL 07 patches it.
...
def test_seattle_does_have_a_slot_in_the_table(tmp_path):
    # The other half of the previous claim, so it cannot be read as "SEA is unknown
    # to this game".
...
def test_a_populated_alias_survives_an_empty_one_arriving_second(tmp_path):
    # Without the guard the empty `SJS` overwrites the populated `SJ`, `patch` skips
    # slot 24, and the run reports success with the 2004 roster still on the disc.
    teams = [
...
def test_a_populated_alias_arriving_second_replaces_an_empty_one(tmp_path):
    # One-sided on purpose: emptiness is what the guard refuses to let win, not
    # lateness.
    teams = [
        Team(id=1, name="San Jose Sharks", code="SJ", logo_url=""),
        Team(id=2, name="San Jose Sharks", code="SJS", logo_url=""),
...
def test_an_empty_roster_that_collides_with_nothing_still_takes_its_slot(tmp_path):
    # The mapped result keeps showing which slots a provider team matched; `patch` is
    # what keeps the empty list away from the writer.
...
def test_mapping_reads_the_leaders_out_of_the_roster(tmp_path):
    # A 90-point season saturates the offensive scale; without the leaders the player
    # takes the centre's default of 35 for `deking`.
    top = squad(1000)
...
def test_patching_counts_every_player_it_placed(tmp_path):
    # Twenty-five rows a team, of which two are goalie rows. The mapped roster has 22
    # players, 2 of them goalies, so all 22 are placed on each of four teams.
    result, _ = patched(tmp_path)
...
def test_a_players_name_reaches_the_bio_the_chain_points_at(tmp_path):
    # The four-hop chain, end to end. Roster row (0, 2) is the first skater row of the
    # first team; its bio lives at a position given by a stride-7 permutation, so a
    # patcher that used the row's own position misses.
...
def test_the_bio_the_chain_points_at_is_not_the_one_at_the_rows_position(tmp_path):
    # Guards the previous test: if the two positions coincided it would pass against a
    # patcher that ignored the chain.
...
def test_a_row_a_short_roster_never_reached_keeps_its_bio(tmp_path):
    # Twenty-two players into twenty-five rows, so rows 22-24 of a patched team are
    # undressed and their bios are left alone.
    _, out = patched(tmp_path)
...
def test_a_goalies_ratings_reach_the_goalie_table(tmp_path):
    # Roster row 0 of team 0 is a goalie row, and a goalie's attributes must go to
    # SGAI because that is the table with a row for him.
    _, out = patched(tmp_path)
...
def test_the_first_paired_player_wears_the_c(tmp_path):
    # Goalies are paired first, so the starting goalie is captain. Unusual on a real
...
def test_the_rows_a_short_roster_did_not_reach_are_undressed(tmp_path):
    # 22 players into 25 rows. The last three rows keep their 2004 occupants and must
    # not take the ice beside a 2025 one.
    _, out = patched(tmp_path)
    rows = rost_of(out)
...
def test_a_patched_row_keeps_its_own_index(tmp_path):
    # Rewriting `INDX` would break the ROST -> PLAY -> SPBT chain that located the row.
    _, out = patched(tmp_path)
...
def test_a_patched_row_has_only_the_flags_the_mapper_gave_it(tmp_path):
    # Every row of the fixture ships with one flag already set, so a patcher that
    # failed to clear the sixty-four would leave it behind.
    _, out = patched(tmp_path)
...
def test_a_skater_row_carries_its_line_and_its_special_teams_unit(tmp_path):
    # Row 2 is the first skater row: the first centre, so first line and first
    # power-play unit.
    _, out = patched(tmp_path)
...
def test_the_third_defence_pair_never_reaches_the_disc(tmp_path):
    ...
    2005's ROST has no `33` anything, so `roster_values` drops the key and the two
    bits the game does name come back zero on every row. Do not "fix" the
    spelling: only a field-name dump from a retail disc could settle which flag
    the game reads, and none may enter this repository.
    """
    _, out = patched(tmp_path)
    rows = rost_of(out)
    third = sorted(
...
def test_no_even_strength_defence_pair_reaches_the_disc(tmp_path):
    # And neither do the first two, so the test above is about defencemen at large.
    # `roster_values` writes all sixty-four flags on every patched row, so these are
    # written zeros and not merely untouched bytes.
    _, out = patched(tmp_path)
    rows = rost_of(out)
    assigned = sorted(
        f
...
def test_the_five_on_three_defence_slots_carry_the_first_two_pairs(tmp_path):
    ...
    Without this, "no even-strength pair was set" would also be satisfied by a
    patch that assigned no defenceman at all.
    """
    _, out = patched(tmp_path)
    rows = rost_of(out)
    set_flags = sorted(
...
def test_the_unreachable_flags_are_all_zero(tmp_path):
    # `X1__` and `X2__` have no counterpart in the mapper at all, so a patched row
    # carries them cleared -- which is a write, not an omission.
    _, out = patched(tmp_path)
    rows = rost_of(out)
    set_flags = [
        f
...
def test_this_game_has_no_bio_mirror_to_write(tmp_path):
    # NHL 07's `nhlbioatt.tdb`. A port that copied its `_MirrorTables` would carry a
    # field that is always None and two writes that never run.
    _, out = patched(tmp_path)
...
def test_a_slot_with_no_rows_on_the_disc_is_not_counted(tmp_path):
    # The source incremented `teams_patched` for every slot it looked at. `CAR` is
    # slot 5 and the fixture disc has no rows for it, so nothing reached the ROM.
    teams = [
...
def test_patching_a_sparse_multi_megabyte_image_works(tmp_path):
    # The copy path over something larger than one 4 MB chunk. `truncate` makes it a
    # hole rather than real storage.
...
def test_an_all_star_slot_is_not_even_named_in_the_progress_messages(tmp_path):
    # The `_write_all_teams` bound is `PATCHABLE_SLOT_COUNT`, not the 32 slots the
    # game has names for, and the two are only distinguishable here: the fixture disc
    # has no ROST rows for slot 30, so a patcher bounded by 32 would report the same
    # `teams_patched` while announcing "Writing East All-Star" to the user.
    source = iso(tmp_path)
    patcher = build(tmp_path, FakeApi())
    mapped = patcher.map_rosters(league())
    mapped.teams[30] = [NHL05PlayerRecord(first_name="Ghost")]

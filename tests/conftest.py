...
way. Of the 66 tests in `tests/games/nhl94_snes/test_patcher.py`, 61 add the
same exposure -- 59 an `EspnClient`, 2 an `NhlApiClient`, none of them both; of
...
Across the whole suite 879 items reach a client built with `transport=None`.
Autouse covers 4758 of the 4766 tests a default run executes; the remaining 8
opt out explicitly. The suite collects 4772 -- `addopts` deselects
`tests/test_packaging.py`'s 6, which are covered too on the run that selects
them -- so 4764 of 4772 counted that way.

Every number in the paragraph above moves when anyone adds a test; this is the
fourteenth commit that has had to re-derive them. Re-derive rather than adjust:
